...
describe('utils.formatZentaoUrl', () => {
    it('formatZentaoUrl', () => {
        expect(utils.formatZentaoUrl('http://demo.zentao.net/')).toBe(
            'http://demo.zentao.net/'
        );
        expect(utils.formatZentaoUrl('http://demo.zentao.net')).toBe(
            'http://demo.zentao.net/'
        );
        expect(utils.formatZentaoUrl('https://demo.zentao.net/')).toBe(
            'https://demo.zentao.net/'
        );
        expect(utils.formatZentaoUrl('https://demo.zentao.net')).toBe(
            'https://demo.zentao.net/'
        );
        expect(utils.formatZentaoUrl('demo.zentao.net')).toBe(
            'http://demo.zentao.net/'
        );
        expect(utils.formatZentaoUrl('demo.zentao.net/')).toBe(
            'http://demo.zentao.net/'
        ...
        expect(utils.formatZentaoUrl('demo.zentao.net/index.php')).toBe(
            'http://demo.zentao.net/'
        );
    });
});
...
describe('utils.slimmingObject', () => {
    it('slimmingObject', () => {
        expect(
            utils.slimmingObject({a: 1, b: null, c: '3', d: true}, ['b', 'c'])
        ).toStrictEqual({b: null, c: '3'});
        expect(
            utils.slimmingObject({a: 1, b: null, c: '3', d: true}, [
                'a',
                'b',
                'c',
                'd',
...
describe('utils.normalizeRequestParams', () => {
    expect(
        utils.normalizeRequestParams({foo: 'bar', hello: 'world'})
    ).toStrictEqual([
        ['foo', 'bar'],
        ['hello', 'world'],
    ]);
    expect(utils.normalizeRequestParams('answer=42')).toStrictEqual([
        ['answer', '42'],
    ]);
